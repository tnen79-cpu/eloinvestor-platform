-- V35 Promotion analytics safety fixes
-- يشغّل بعد V34.9.
-- الهدف: العدّادات كلها Atomic داخل SQL بدون قراءة ثم كتابة من الواجهة.
...
create or replace function public.increment_active_promotion_metric_atomic(p_project_id text, metric_name text)
returns void
language plpgsql
security definer
...
-- الاسم الأساسي يبقى موجودًا ويستدعي نفس الدالة الآمنة حتى لا ينكسر الكود القديم.
...
set search_path = public
as $$
  select public.increment_active_promotion_metric_atomic(p_project_id, metric_name);
$$;

grant execute on function public.increment_active_promotion_metric_atomic(text, text) to anon, authenticated;
grant execute on function public.increment_active_promotion_metric(text, text) to anon, authenticated;

-- عدّاد مشاهدات المشروع العام Atomic أيضًا، بدل read -> update من الواجهة.
...
begin
  ...
  update public.projects
     set views_count = coalesce(views_count, views, 0) + 1,
         views = coalesce(views_count, views, 0) + 1
   where id = v_project_uuid;
end;
$$;

grant execute on function public.increment_project_view_count(text) to anon, authenticated;
