...
type Tab = 'overview' | 'projects' | 'sectors' | 'verifications' | 'packages' | 'upgrades' | 'recommendations' | 'notifications' | 'users' | 'admins' | 'ads' | 'countries' | 'slides' | 'analytics' | 'reports' | 'ratings' | 'logs' | 'seo';
...
function statusMeta(status: string) {
  ...
  if (['approved', 'active', 'published'].includes(s)) return { label: 'منشور', cls: 'bg-emerald-50 text-emerald-700 ring-emerald-100', icon: CheckCircle2 };
...
export default function AdminDashboard() {
  ...
  async function loadAll() {
    ...
    const [projectResult, countriesResult, slidesResult, adsResult, sectorsResult, campaignsResult, sliderSettingResult, usersResult, packagesResult, verificationResult, upgradesResult, notificationsResult, adminInvitesResult, reportsResult, ratingsResult, logsResult, savesCount, contactedCount, conversationsCount, messagesCount, viewsCount] = await Promise.allSettled([
      ...
      supabaseBrowser.rpc('admin_get_reports'),
      supabaseBrowser.rpc('admin_get_ratings'),
      supabaseBrowser.rpc('admin_get_logs'),
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      revenue: packages.reduce((sum, p) => sum + (p.isActive ? p.price : 0), 0),
    };
  }, [projects, users, verifications, upgrades, packages, reports, ratings, activityLogs]);
  ...
  async function sponsorProject(project: AdminProject) {
    ...
    if (loading) return <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6"><div className="rounded-[2rem] bg-white p-8 text-center shadow-sm"><Loader2 className="mx-auto h-8 w-8 animate-spin text-emerald-600" /><p className="mt-4 font-black text-slate-700">جاري تحميل لوحة الإدارة...</p></div></div>;
  if (accessDenied) return <div className="flex min-h-screen items-center justify-center bg-slate-950 p-6 text-white"><div className="max-w-lg rounded-[2rem] bg-white/10 p-8 text-center backdrop-blur"><LockKeyhole className="mx-auto h-12 w-12 text-amber-300" /><h1 className="mt-4 text-3xl font-black">صلاحية الإدارة مطلوبة</h1><p className="mt-3 text-slate-300">تأكد أن حسابك يحمل role = admin أو super_admin.</p><Link className="mt-6 inline-flex rounded-full bg-emerald-500 px-6 py-3 font-black text-slate-950" href="/om/ar/login">تسجيل الدخول</Link></div></div>;
  ...
  const tabs: { key: Tab; label: string; icon: any }[] = [
    ...
    { key: 'sectors', label: 'القطاعات', icon: Layers3 },
    { key: 'verifications', label: 'التوثيق', icon: ShieldCheck },
    { key: 'packages', label: 'الباقات', icon: Crown },
    { key: 'upgrades', label: 'طلبات الترقية', icon: PackageCheck },
    { key: 'recommendations', label: 'التوصيات', icon: Sparkles },
    { key: 'notifications', label: 'الإشعارات', icon: Bell },
    { key: 'reports', label: 'البلاغات', icon: Flag },
    { key: 'ratings', label: 'التقييمات والثقة', icon: Star },
    { key: 'logs', label: 'سجل الإدارة', icon: FileText },
    { key: 'seo', label: 'SEO', icon: ShieldAlert },
    { key: 'analytics', label: 'التحليلات', icon: Activity },
    { key: 'users', label: 'المستخدمين', icon: Users },
    { key: 'admins', label: 'حسابات الإدارة', icon: UserPlus },
    { key: 'ads', label: 'الإعلانات والتمويل', icon: Megaphone },
    { key: 'countries', label: 'الدول', icon: Globe2 },
    { key: 'slides', label: 'السلايدر', icon: SlidersHorizontal },
  ];
    ...
    <main dir="rtl" className="min-h-screen bg-slate-50 text-slate-950">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-6 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-black text-emerald-600">EloInvestor Admin Control Center</p>
            <h1 className="mt-1 text-3xl font-black md:text-4xl">لوحة تحكم الإدارة</h1>
            <p className="mt-2 text-sm font-bold text-slate-500">مرحبًا {admin?.name} — تحكم كامل بالمميزات والباقات والتوثيق والتوصيات.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={loadAll} className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-4 py-3 text-sm font-black text-slate-700 hover:bg-slate-200"><RefreshCcw className="h-4 w-4" /> تحديث</button>
            <Link href="/om/ar" className="rounded-full bg-emerald-600 px-5 py-3 text-sm font-black text-white">فتح الموقع</Link>
          </div>
        </div>
      </section>

      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[260px_1fr]">
        <aside className="rounded-[2rem] border border-slate-100 bg-white p-3 shadow-sm lg:sticky lg:top-4 lg:h-fit">
          <div className="grid gap-2">
            {tabs.map((item) => (
              <button key={item.key} onClick={() => setTab(item.key)} className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-right text-sm font-black transition ${tab === item.key ? 'bg-slate-950 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
                <item.icon className="h-5 w-5" /> <span className="flex-1">{item.label}</span>{tabBadge(item.key) ? <span className="rounded-full bg-amber-400 px-2 py-0.5 text-[10px] font-black text-slate-950">{tabBadge(item.key)}</span> : null}
              </button>
            ))}
          </div>
        </aside>
        ...
        <section className="min-w-0">
          {notice ? <div className={`mb-5 rounded-2xl p-4 text-sm font-black ring-1 ${notice.type === 'error' ? 'bg-red-50 text-red-700 ring-red-100' : notice.type === 'success' ? 'bg-emerald-50 text-emerald-700 ring-emerald-100' : 'bg-sky-50 text-sky-700 ring-sky-100'}`}>{notice.text}</div> : null}
          ...
          {tab === 'overview' && (
            <div className="grid gap-5">
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <AdminStat icon={BriefcaseBusiness} label="كل المشاريع" value={stats.projects} tone="bg-emerald-50 text-emerald-700" />
                ...
                <AdminStat icon={ShieldCheck} label="طلبات توثيق معلقة" value={stats.verifications} tone="bg-emerald-50 text-emerald-700" />
              ...
              <div className="grid gap-5 xl:grid-cols-3">
                <div className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm xl:col-span-2">
                  ...
                  <div className="mt-4 grid gap-3">
                    ...
                    <button onClick={() => setTab('upgrades')} className="rounded-2xl bg-emerald-50 p-4 text-right font-black text-emerald-800">{stats.upgrades} طلب ترقية باقة بحاجة مراجعة</button>
                ...
                {filteredProjects.map((project) => {
                  ...
                  return <article key={project.id} className="rounded-[2rem] border border-slate-100 bg-white p-4 shadow-sm">
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2"><button onClick={() => updateProject(project.id, { status: 'approved' })} className="rounded-full bg-emerald-600 px-3 py-2 text-xs font-black text-white">نشر</button><button onClick={() => updateProject(project.id, { is_verified: true, verification_status: 'approved' })} className="rounded-full bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">توثيق</button><button onClick={() => updateProject(project.id, { is_featured: true, featured_until: new Date(Date.now() + 7 * 86400000).toISOString() })} className="rounded-full bg-amber-50 px-3 py-2 text-xs font-black text-amber-700">تمييز 7 أيام</button><button onClick={() => sponsorProject(project)} className="rounded-full bg-yellow-100 px-3 py-2 text-xs font-black text-yellow-800">تمويل المشروع</button><button onClick={() => updateProject(project.id, { status: 'revision' })} className="rounded-full bg-orange-50 px-3 py-2 text-xs font-black text-orange-700">طلب تعديل</button><button onClick={() => updateProject(project.id, { status: 'rejected' })} className="rounded-full bg-rose-50 px-3 py-2 text-xs font-black text-rose-700">رفض</button><button onClick={() => openProjectEditor(project)} className="rounded-full bg-blue-50 px-3 py-2 text-xs font-black text-blue-700">تعديل</button><Link target="_blank" href={`/${project.countryCode || 'om'}/ar/project/${encodeURIComponent(project.slug || project.id)}?adminPreview=1`} className="rounded-full bg-slate-100 px-3 py-2 text-xs font-black text-slate-700"><Eye className="inline h-3 w-3" /> معاينة</Link><button onClick={() => deleteProject(project.id)} className="rounded-full bg-slate-950 px-3 py-2 text-xs font-black text-white"><Trash2 className="inline h-3 w-3" /></button></div>
                    </div>
                  </article>;
                })}
              </div>
          ...
          {tab === 'ads' && <AdsPanel ads={ads} campaigns={campaigns} projects={projects} form={adForm} setForm={setAdForm} campaignForm={campaignForm} setCampaignForm={setCampaignForm} saveAd={saveAd} deleteAd={deleteAd} saveCampaign={saveCampaign} deleteCampaign={deleteCampaign} sponsorProject={sponsorProject} />}
      ...
      {projectEditModal ? <ProjectEditDialog modal={projectEditModal} setModal={setProjectEditModal} onSave={saveProjectEditor} onClose={() => setProjectEditModal(null)} addImage={addProjectImage} deleteImage={deleteProjectImage} /> : null}
    </main>
  );
}
    ...
    {reports.length ? reports.map((item: ReportRow) => <article key={item.id} className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => reviewReport(item, 'resolved')} className="rounded-full bg-emerald-600 px-4 py-3 text-sm font-black text-white">إغلاق</button>
  ...
  return <div className="grid gap-5">
    <div className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm">
      ...
      <form onSubmit={saveSector} className="mt-5 grid gap-3 md:grid-cols-3">
        ...
        <button className="rounded-2xl bg-emerald-700 px-5 py-3 font-black text-white">حفظ القطاع</button>
      </form>
    </div>
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {sectors.length ? sectors.map((sector: SectorRow) => <div key={sector.id || sector.key} className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3">
          {sector.imageUrl ? <div className="h-14 w-14 rounded-2xl bg-cover bg-center" style={{ backgroundImage: `url(${sector.imageUrl})` }} /> : <div className="grid h-14 w-14 place-items-center rounded-2xl bg-emerald-50 text-2xl">{sector.icon}</div>}
...
function AdsPanel({ ads, campaigns, projects, form, setForm, campaignForm, setCampaignForm, saveAd, deleteAd, saveCampaign, deleteCampaign, sponsorProject }: any) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
    <form onSubmit={saveSlide} className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 grid gap-3">
        ...
        <input value={form.buttonTextAr} onChange={(e) => setForm((f: any) => ({ ...f, buttonTextAr: e.target.value }))} placeholder="نص الزر" className="admin-input" />
        <input value={form.buttonUrl} onChange={(e) => setForm((f: any) => ({ ...f, buttonUrl: e.target.value }))} placeholder="رابط الزر" className="admin-input" />
        ...
        <button className="rounded-full bg-emerald-600 px-5 py-3 font-black text-white">حفظ السلايد</button>
      </div>
    </form>
    <div className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-sm">
        ...
        {slides.length ? slides.map((slide: SlideRow) => <div key={slide.id || `${slide.countryCode}-${slide.slideOrder}`} className="rounded-2xl bg-slate-50 p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => toggleSlide(slide)} className={`rounded-full px-3 py-2 text-xs font-black ${slide.isActive ? 'bg-amber-50 text-amber-800' : 'bg-emerald-50 text-emerald-700'}`}>{slide.isActive ? 'إيقاف' : 'تشغيل'}</button>
