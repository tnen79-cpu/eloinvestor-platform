  ...
  async function submitReport() {
    ...
    const response = await fetch('/api/trust/report', {
      ...
      body: JSON.stringify({
        targetType,
        targetId,
        projectId: projectId || (targetType === 'project' ? targetId : null),
    ...
    if (!response.ok || !payload.ok) {
      setError(payload.error || (isAr ? 'تعذر إرسال البلاغ.' : 'Failed to submit report.'));
      return;
    }
    setDone(true);
    setDescription('');
      ...
      {open ? (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" dir={isAr ? 'rtl' : 'ltr'}>
          <div className="w-full max-w-lg rounded-[2rem] bg-white p-6 shadow-2xl">
            ...
            {done ? <div className="mt-5 rounded-2xl bg-blue-50 p-4 text-sm font-black text-blue-700">{isAr ? 'تم إرسال البلاغ بنجاح.' : 'Report submitted successfully.'}</div> : (
              <div className="mt-5 grid gap-3">
                <select value={reason} onChange={(e) => setReason(e.target.value)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 font-bold outline-none focus:border-blue-500">
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="min-h-32 rounded-2xl border border-slate-200 bg-white px-4 py-3 font-bold outline-none focus:border-blue-500" placeholder={isAr ? 'اكتب تفاصيل البلاغ...' : 'Add report details...'} />
