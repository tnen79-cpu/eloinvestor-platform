  ...
  return (
    <section className="sponsored-strip">
      ...
      <div className="sponsored-grid">
        {items.map((p) => <Link key={p.id || p.slug} href={`/${country.code}/${lang}/project/${p.slug || p.id}`} className="sponsored-card"><img src={p.image} alt="" /><div><b>{isAr ? p.titleAr : p.titleEn}</b><span>{formatMoneyForCountry(p.price, country, lang)} · {p.roi}%</span></div><i>{isAr ? 'ممول' : 'Sponsored'}</i></Link>)}
      </div>
    </section>
  );
}
