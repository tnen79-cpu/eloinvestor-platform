...
import { Chrome, Smartphone, CheckCircle2, UserRound } from 'lucide-react';
...
const ACCOUNT_TYPES = [
  ...
  { value: 'both', ar: 'مستثمر وصاحب مشروع', en: 'Investor & owner' },
];
...
export function RegisterForm({ country, lang }: { country: string; lang: string }) {
  ...
  // ✅ FIX 2: accountType state is never reset — user selection is always preserved
  ...
  async function finishRegister(payload: Record<string, unknown> = {}) {
    // ✅ FIX 2: Always pass the user-selected accountType explicitly
  ...
  async function sendOtp(event: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage('');
    try {
      if (!hasFirebaseConfig()) throw new Error(isAr ? 'مفاتيح Firebase غير مضافة في المشروع.' : 'Firebase keys are missing.');
  ...
  async function registerWithGoogle() {
    ...
    try {
      ...
      // ✅ FIX 2: Pass user-selected accountType to Google registration too
      await syncFirebaseProfile({
        name: googleName,
        account_type: accountType,
        complete_onboarding: true,
  ...
  return (
    <div className="mt-8 space-y-4">
      <p className="text-center text-[11px] font-black text-blue-500/70">Firebase Auth v87</p>

      {/* ✅ FIX 3: reCAPTCHA container — always visible, cleared between attempts */}
      ...
      <input
        ...
        placeholder={t('auth', 'full_name', isAr ? 'الاسم الكامل' : 'Full name')}
        required={step === 'phone'}
      />

      {/* ✅ FIX 2: Visual account type selector — never resets, always explicit */}
      <div>
        <p className="mb-2 text-sm font-black text-slate-700">{isAr ? 'نوع الحساب' : 'Account type'}</p>
          ...
          {ACCOUNT_TYPES.map((type) => (
            ...
            >
              {accountType === type.value
                ...
                : <UserRound className="mx-auto mb-1 text-blue-500" size={16} />}
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white px-6 py-4 font-black text-slate-900 shadow-sm transition hover:border-blue-300 hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-60"
      >
        <Chrome size={21} />
      ...
      {step === 'phone' ? (
        <form onSubmit={sendOtp} className="space-y-4">
          <div className="grid grid-cols-[130px_1fr] gap-3">
            <select
              ...
              className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-4 font-bold outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10"
          ...
          <button
            disabled={loading || !name.trim() || phone.length < 6}
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-700 px-6 py-4 font-black text-white shadow-lg shadow-blue-900/10 disabled:cursor-not-allowed disabled:opacity-60"
          >
            <Smartphone size={19} />
        ...
        <form onSubmit={verifyOtp} className="space-y-4">
          <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700">
            ...
            <span dir="ltr" className="text-blue-700">{fullPhone}</span>
          </div>
          <input
            value={otp}
            ...
            pattern="[0-9]*"
            maxLength={6}
            className="w-full rounded-2xl border border-slate-200 px-5 py-4 text-center text-2xl font-black tracking-[0.35em] outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10"
            placeholder="000000"
            required
          />
          {!!message && <p className="rounded-2xl bg-blue-50 px-4 py-3 text-sm font-bold text-blue-800">{message}</p>}
          <button
            ...
            className="w-full rounded-2xl bg-blue-700 px-6 py-4 font-black text-white shadow-lg shadow-blue-900/10 disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          <button
            ...
            className="w-full rounded-2xl bg-slate-100 px-6 py-3 font-black text-slate-700"
          >
      ...
      <Link href={`/${country}/${lang}/login`} className="block text-center font-bold text-blue-700">
        {t('auth', 'already_have_account', isAr ? 'عندي حساب بالفعل' : 'I already have an account')}
      </Link>
    </div>
  );
}
