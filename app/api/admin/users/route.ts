...
function validAdminCookie(raw: string | undefined) {
  ...
  try {
    return timingSafeEqual(Buffer.from(sig), Buffer.from(sign(value)));
  } catch {
    return false;
  }
}
...
async function allowedByFirebase(request: NextRequest, db: any) {
  ...
  if (uid) await collect(db.from('users').select('*').eq('firebase_uid', uid).limit(5));
  if (email) await collect(db.from('users').select('*').ilike('email', email).limit(5));
  if (phone) await collect(db.from('users').select('*').eq('phone', phone).limit(5));
...
export async function GET(request: NextRequest) {
  try {
    if (!supabaseUrl || !serviceRoleKey) {
      return NextResponse.json({ ok: false, error: 'SUPABASE_SERVICE_ROLE_KEY_MISSING' }, { status: 500 });
    }
    const db = createClient(supabaseUrl, serviceRoleKey, { auth: { persistSession: false, autoRefreshToken: false } });
    const cookieOk = validAdminCookie(request.cookies.get(COOKIE_NAME)?.value);
    const firebaseOk = cookieOk ? false : await allowedByFirebase(request, db).catch(() => false);
    if (!cookieOk && !firebaseOk) return NextResponse.json({ ok: false, error: 'FORBIDDEN' }, { status: 403 });
